...
modDatetime: 2026-08-31T23:05:20.000+08:00
...
rehype-callouts检测到`.dark`类切换暗色模式，但Paper使用 `data-theme="dark"` 属性切换暗色模式。所以`.dark .callout` 选择器永远匹配不到，callout文字样式一直处于浅色模式状态， 而`mix-blend-mode: darken` 在暗色模式下会将浅色文字混合为深色，文字完美融入背景，造成callout块内部无文字的假象。
    
修复方法：在`global.css`中禁用`.callout`的`mix-blend-mode`属性，一劳永逸

修复方法：在 `global.css` 的 @layer 外部设置 `.callout { mix-blend-mode: normal }` 。必须放在 @layer 外，因为Callout库的CSS无前置层级，无 layer 样式优先级高于 @layer 内样式。
...
> 这是一条信息
> 
> 支持的语法包括: NOTE, ABSTRACT, INFO, TODO, TIP, SUCCESS, QUESTION, WARNING, FAILURE, DANGER, BUG, EXAMPLE, QUOTE 
> 
