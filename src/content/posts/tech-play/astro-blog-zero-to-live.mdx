...
modDatetime: 2026-09-11T00:45:20.000+08:00
title: 从零到上线，迁移和修改一个Astro博客全过程
featured: true
draft: false
tags:
  - Astro
...
> [!NOTE]- 修订记录
> - 2026-06-21，完成初版并发布
> - 2026-08-31，修订排版，添加一只小猫，它很可爱！
> - 2026-09-03，添加自定义博客部分的内容
> - 2026-09-11，修改最后的工作流部分
...
<Collapse title="没什么，只是折叠了一只吃高闪的小猫，以防你在深夜打开被闪到">

  <figure>
    <img class="w-2/3 md:w-1/3" src="https://img.055933.xyz/file/1788189044097_giphy__2_.gif" alt="cat-flashbang.gif" />
    <figcaption>调试Cactus那天晚上的我</figcaption>
  </figure>
...
配置中的editPost指的是文章页面的 `Edit post` 按钮，点击会跳转到Github上这篇文章对应的编辑页面，前提是你是仓库的使用者并且已经登录。
...
基本格式是`https://github.com/username/仓库名/edit/对应分支/`
...
我觉得默认的英文字体很好看，所以能保留的地方我都保留了。汉化具体方法：在 `src/i18n/lang` 文件夹下复制`en.ts` 文件，改名为 `zh-CN.ts` ，对照 `en.ts` 按需翻译为中文

也有些地方是无法通过这种方式汉化的，这种时候只能全站搜索改源码，但Astro Paper的结构很清晰，改起来也方便，除了主题引入的目录功能，它默认的目录生成方式是在需要生成的地方输入一个二级标题 `##Table of contents` ，但是它在生成以后也叫这个，无法直接翻译，要翻译得改这个组件的源代码，我觉得没必要，就保留了。
...
于是让Pi Coding Agent写了一个remark插件，自动在中英文之间添加一个瘦空格，视觉效果上比等宽空格要好很多，而且不用手动添加空格了
...
8月31日新增：我完善了这个插件，现在除了在中英文之间添加空格，行内代码块与周围、链接文本与周围文字也会自动添加空格，我试用觉得很完美。如果你想试试可以下载我的

import DownloadButton from "@/components/DownloadButton.astro"

<DownloadButton
  href="https://raw.githubusercontent.com/coyoteshkw/coyotec-blog/refs/heads/main/src/utils/remarkCjkSpacing.ts"
  filename="remarkCjkSpacing.ts"
/>

食用方法：在`astro.config.ts`文件中作为remarkPlugin引入
...
最大的问题出在，样式不生效，不管我干什么，总之没效果

经过大力气后发现，不是真的没生效，而是在Paper的「暗黑模式」不生效

因为rehype-callouts检测到`.dark`类切换暗色模式，但Paper使用 `data-theme="dark"` 属性切换暗色模式。所以`.dark .callout` 选择器永远匹配不到，callout样式一直处于默认的`mix-blend-mode: darken`状态，而`mix-blend-mode: darken` 在暗色模式下会将浅色文字混合为深色，文字完美融入背景，造成callout块内部无文字的假象。

修复方法：在`global.css`中禁用`.callout`的`mix-blend-mode`属性，一劳永逸。记得放在`@layer`外，否则会被覆盖
...
同时修改`details`选择器为`details:not(.callout)`，避免可折叠 callout 内文字被隐藏。
...
我觉得默认的字体有点大，在`global.css`内改小了1px，顺便把无序列表的marker小标配色改了，否则会跟随主题配色，在callout内显得有些突兀

```css
.callout {
  /* 解决callout在深色模式下颜色不正确的问题 */
  mix-blend-mode: normal;
  .callout-content {
    font-size: 15px;
  }
  /* callout内部的无序列表不跟随博客整体配色 */
  li::marker {
    @apply text-current
  }
}
...
> 这是一条信息
>
> 在导入全部样式的情况下，支持的语法包括: NOTE, ABSTRACT, INFO, TODO, TIP, SUCCESS, QUESTION, WARNING, FAILURE, DANGER, BUG, EXAMPLE, QUOTE
>
...
原博手机端~~直接没有点赞了~~在我看的时候还没有手机端的点赞按钮，我自己写了一个放在文末。再额外写了一个小功能：文章浏览过半时，点赞按钮会弹出一个气泡框鼓励用户交互^\_^，如果用户点击了就会提示谢谢~然后消失。如果什么都不做等10s后也会消失。移动端只有划到底部按钮可视时才会提示可点赞，因为本身就有一个回到顶部+目录按钮，再加一个有点过多了，影响体验
...
mdx可以引入组件，但偶尔用md会更方便

:link[remark-directive-sugar]{id=https://github.com/lin-stephanie/remark-directive-sugar}是一个remark插件，拓展了默认md的功能

提供 `:link`、`:badge`、`::video-`、`:::image-` 四种拓展语法，具体可以看这篇[测试文章](https://coyoteshkw.com/posts/tech-play/remark-directive-sugar-test/)，支持链接按钮、徽标、视频、图片注解，很适合轻量使用

自定义链接按钮会显示favicon，我换了默认的duckduckgo favicon接口，可以在中国大陆地区接收图标，但是其他的默认Github图标就不太好改了，所以我只使用下列语法来展示链接按钮
...
本功能完全参照夏叶大佬的[这篇文章](https://blog.cuteleaf.cn/posts/dev-notes/astro-blog-recommend-algorithm/)实现，只是在样式上做了本地化处理，并且我认为处理的还有纰漏，做的并不好

该功能会根据标签、标题、发布时间等属性排版出相似度最高的5篇文章，在文末的See also部分展示

![image.png](https://img.055933.xyz/file/1788194072560_image.png)
...
## 附录1：重定向
我在提交Cloudflare自带域名到Google Console之后很久才购买的域名，因此前域名已经被收录了，直接下掉不好，通过301重定向的方式先把流量分过去，以后再考虑删除默认域名。
...
再创建一个「批量重定向规则」（刚才那个是批量重定向规则「列表」），选择刚才创建的列表，保存并部署即可。几分钟后可以测试是否成功重定向

![image.png](https://img.055933.xyz/file/1782031196734_image.png)

即使你没有提交自带域名，也可以执行此步骤来提高主域名的权重
...
## 附录3：个人写文章流程
基本过程：<u> ~~files.md~~ markedit内写完 → neovim内看排版 → 博客内格式精修 → 发布</u>

<Collapse title="修订前内容">
...
MarkEdit是一个MacOS原生MD编辑工具，并非所见即所得，也没有高级的功能，但是很适合快速编写和检查排版

由于一种我也不知道是什么的效应，在手机上检查文章会比在电脑上更加容易发现问题，我会时不时通过手机回看我的文章，发现哪里不对就直接截图，用编辑器手动圈一下，等到上机时翻出来统一修订

初版会通过tiddlywiki或silverbullet进行存档，后续对博客的更新不会同步过去除非有重大改动，避免一来一回耗费精力
